...
from mngr.views import get_ip, send_sms
...
def rannum(size=8, chars=string.digits):
    return ''.join(random.choice(chars) for _ in range(size))

#generate random letters
def ranlet(size=2, chars=string.ascii_uppercase):
    return ''.join(random.choice(chars) for _ in range(size))
...
def profile(request):
    ...
    if SpecialCase.objects.filter(regnumber=str(userprofile.regnum), noresult=True).exists():
        dtruant = SpecialCase.objects.filter(regnumber=str(userprofile.regnum), noresult=True)[0]
        specialcase = True
    else:
        dtruant = False
        specialcase = False
    ...
    card = AccessCode.objects.get(srn=userprofile.serial)

    if card.custom:
        try:
            ucenter = ExamCenter.actv.get(name=card.custom_center)
        except ExamCenter.DoesNotExist:
            pass
        else:
            userprofile.exam_center = ucenter
            userprofile.save()
            dcenter = ExamCenter.objects.get(name=ucenter)
        ...
        area = ExamArea.objects.get(name=userarea)
    ...
    if card.freebie == True:
        if userprofile.give_sample == False:
            samples = Freebie.objects.filter(active=True, used=False)
            try:
                sample = samples[0]
            except Exception as e:
                pass
            else:
                sample.user = str(userprofile.regnum)
                sample.used = True
                sample.save()
                userprofile.give_sample = True
                userprofile.giftpin = sample.pin
                userprofile.giftserial = sample.srn
                userprofile.save()

    gift = None
    try:
        gift = Freebie.objects.get(srn=userprofile.giftserial)
    except Freebie.DoesNotExist:
        gift = None
    if not userprofile.exam_center:
        centerd = None
    else:
        centerd = ExamCenter.objects.get(name=userprofile.exam_center)
...
def result(request, pk):
    ...
    if userprofile.exam_center:
        loc = ExamCenter.objects.get(name=userprofile.exam_center)
    else:
        loc = False
    try:
...
# def reg_refer_profile(pk, request):
#     today = timezone.now().today()
 #   CURRENTYEAR = today.year
#     if request.method == 'POST':
#         uform = UserForm(request.POST)
...
#             return render(request, 'userprofile/referral.html', context_instance)
#     else:
#         get_ip(request, 'register')
#         uform = UserForm()
#         pform = UserProfileForm()
#         sform = SubjectForm()
#     return render(request, 'userprofile/referral.html', {'uform': uform, 'pform': pform, 'sform': sform, 
#'CURRENTYEAR':CURRENTYEAR})
