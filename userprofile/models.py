...
class ExamArea(models.Model):
    ...
    def __str__(self):
        if self.exam_period:
            return "%s, %s %s" %(
                self.name, 
                self.exam_period.strftime('%b'), 
                self.exam_period.year)
        else:
...
class UserProfile(models.Model):
    ...
    exam_state = models.ForeignKey(
        State, on_delete=models.PROTECT)
    exam_area = models.ForeignKey(ExamArea, on_delete=models.PROTECT)
    subject = models.ManyToManyField(Subject)
    passport = models.ImageField(
        upload_to='passport/%Y/%m', 
        help_text=("Maximum of 250kb.(50x50px)"))
    regnum = models.CharField(max_length=10)
    viewed_photocard = models.BooleanField(default=False)
    exam_center = models.ForeignKey(
        ExamCenter, on_delete=models.SET_NULL, 
        blank=True, null=True,)
    feedback = models.ForeignKey(
        Advert, on_delete=models.SET_NULL, 
        blank=True, null=True, 
        help_text=("How did you hear about this mock?"))
    batch = models.ForeignKey(
        Batch, on_delete=models.SET_NULL, blank=True, null=True)
    seat = models.IntegerField(
        null=True, blank=True)
    is_online = models.BooleanField(default=False)
    regdate = models.DateTimeField(auto_now_add=True)
    # give_sample = models.BooleanField(default=False)
    # giftpin = models.CharField(
    # max_length=10, default='', null=True, blank=True) 
    # giftserial = models.CharField(
    # max_length=10, default='', null=True, blank=True)
    # payment_mode = models.CharField(
    # max_length=8, choices=PAYMENT, default='card') 
    ...
    # def image_tag(self):
    #     return mark_safe(
    #         '<img src="/media/%s" width="80" height="80" />' % (
    #             self.passport))
    ...
    # # def buyer(self):
    # #     try:
    # #         cd = AccessCode.objects.get(
    # # srn=self.serial, pin=self.pin)
    # #         return cd.buyer
    # #     except Exception:
    # #         return '-'
    ...
    #                                 examination=str(exam), 
    #                                 subject=str(subject.name), 
    #                                 topic=str(tp), 
    #                                 subtopic=str(i)
    #                                 ).order_by('?')[:(int(i.to_do))]
    #                             bulk_script = []
    #                             for ques in que_set:
    ...
    #             'question':q.question, 
    #             'selected_choice':q.choice
    #             } for q in UserScript.objects.filter(
    #             user = self)]
    #     return questions

    # def result_score(self):
    #     if self.taken_test() == "True":
    #         taken = Result.objects.get(
    #             user__regnum=self.regnum)
    #         return taken.total
    #     else:
    #         return "Not Taken"
...
class Subscription(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=7, decimal_places=2, default=2000.00)
...
#     user = models.OneToOneField(
#         UserProfile, on_delete=models.CASCADE)
#     exam_area = models.ForeignKey(
#         ExamArea, on_delete=models.PROTECT)
#     exam_center = models.ForeignKey(
#         ExamCenter, on_delete=models.PROTECT)
#     batch = models.ForeignKey(
...
#     def card(self):
#         sn = AccessCode.objects.get(
#             srn=self.user.serial, pin=self.user.pin)
#         return sn.buyer
...
#         if ss.exists() == True:
#             for i in ss:
#                 dscore = str(i.subject) + ' = ' 
#                 + str(int(i.score)) 
#                 #+ ' [attempted('+ str(i.attempted()) +' questions)]'
#                 s.append(dscore)
#         return s
...
#             for i in ss:
#                 dscore = str(i.subject) + ' = ' 
#                 + str(int(i.score)) + ' [attempted('
#                 + str(i.attempted()) +' questions)]'
#                 s.append(str(dscore))
#         return s
...
#     #         # if 
#     # (
#     # instance.force_markscript != True 
#     # and 
#     # instance.timelog >= timezone.make_aware(
#     # datetime.strptime(
#     # "2021-05-01", "%Y-%m-%d"), 
#     # timezone.get_default_timezone()) 
#     # and instance.timelog <= timezone.make_aware(
#     # datetime.strptime(
#     # "2021-06-01", "%Y-%m-%d"), 
#     # timezone.get_default_timezone())):
#     #         if not instance.marked:
#     #         # if instance.timelog >= timezone.make_aware(
#     # datetime.strptime(
#     # "2021-05-01", "%Y-%m-%d"), 
#     # timezone.get_default_timezone()) 
#     # and instance.timelog <= timezone.make_aware(
...
#     # timezone.get_default_timezone()):
#     #             oldscore = SubjectScore.objects.filter(
#     # user=str(instance.user.regnum))
#     #             for j in oldscore:
#     #                 j.delete()
#     #             exam = instance.user.examination
...
#     #                 try:
#     #                     subj, new = SubjectScore.objects.get_or_create(
#     # user=str(instance.user.regnum), subject=str(p.subject)) 
#     #                     dtpc = p.topic  
#     #                     dsub = p.subject
#     #                     dsubtop = p.subtopic
#     #                     sbjct = Subject.objects.get(
#     # name=dsub, examination=exam)
#     #                     try:
#     #                         tpc = Topic.objects.get(
#     # name=str(dtpc), subject=sbjct)
#     #                     except Topic.DoesNotExist:
#     #                         wet = sbjct.weight
#     #                     else:
#     #                         try:
#     #                             sbtp = SubTopic.objects.get(
#     # name=str(dsubtop), topic=tpc)
#     #                             wet = sbtp.weight
...
#     #             total = 0
#     #             scripts_total = 0
#     #             for sub in SubjectScore.objects.filter(
#     # user=str(instance.user.regnum)):
...
#             exam = instance.user.examination
#             timespent = timezone.localtime(
#                 timezone.now()) - instance.timestamp
#             time_duration = timedelta(minutes= int(exam.duration))
...
#         subjt = Subject.objects.get(
#             name=self.subject, examination=exam)
#         # self.score = 0
#         # self.save()
#         scripts_total = 0
#         scripts = UserScript.objects.filter(
#             user=user, question__subject=subjt.name)
#         for k in scripts:
#             if k.is_right():
...
#         user = UserProfile.objects.get(regnum=self.user)
#         count = 0
#         for i in UserScript.objects.filter(
#             user=user, question__subject=self.subject):
#             if i.is_right():
#                 count += 1
#         return count
...
#remark script
# subj, new = SubjectScore.objects.get_or_create(
# user=str(person.user.regnum), subject=str(p.subject)) 
#             dtpc = p.topic  
#             dsub = p.subject
#             dsubtop = p.subtopic
...
#         State, on_delete=models.SET_NULL, blank=False, null=True)
#     ref_code = models.CharField(max_length=5, unique=True)
#     regdate = models.DateTimeField(auto_now_add=True)
#     account_no = models.CharField(
#         max_length=10, 
#         verbose_name="Acount Number", 
#         blank=True, null=True)
#     bank = models.CharField(
#         max_length=100, 
#         verbose_name="Bank", blank=True, null=True)
#     account_name = models.CharField(
#         max_length=100, blank=True, null=True)
#     referrer_code = models.CharField(
#         max_length=4, null=True, blank=True, 
#         verbose_name="Code Of Referrer")
...
#     user_activated = models.ForeignKey(
#         UserProfile, on_delete=models.PROTECT)
#     referrer = models.ForeignKey(Referrer, on_delete=models.CASCADE)
#     active = models.BooleanField(
# default=False, verbose_name="Candidate Paid?", 
# help_text="has the referred candidate paid?")
#     paid = models.BooleanField(
# default=False, help_text="has the referrer been paid?")
...
#     sup_referrer = models.ForeignKey(
# Referrer, on_delete=models.PROTECT)
#     referral = models.ForeignKey(Referral, on_delete=models.CASCADE)
#     paid = models.BooleanField(
# default=False, help_text="has the sup_referrer been paid?")
#     payment_date = models.DateTimeField(blank=True, null=True)
...
#     if created:
#         try:
#             sup_ref = Referrer.objects.get(
# ref_code=instance.referrer.referrer_code)
#             profile, new = SupReferral.objects.get_or_create(
# sup_referrer=sup_ref, referral=instance)
...
# max_length=50, help_text='Or Teller No.')
